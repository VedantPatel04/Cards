...
def ingest_card_catalog():
    # anchored to BASE_DIR (backend/) so this works no matter what directory the caller runs from
    file_path = os.path.join(settings.BASE_DIR, 'data', 'card_catalog', 'card_catalog.json')
    with open(file_path, 'r') as file:
        card_data = json.load(file) # load card catalog data into list of dicts
